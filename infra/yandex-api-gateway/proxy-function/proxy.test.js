...
test('the allowlist has exactly the entries it is supposed to have', () => {
  ...
  assert.deepEqual(listed, [
    ...
    'POST /api/v1/retail/promo/quote',
    // Telegram Mini App, added in B-6. Two, not the fourteen the architecture
    // lists: the other twelve have no handler upstream yet.
...
test('the Allow-Headers list is exactly what the storefront sends', () => {
  // Authorization added in B-6: without it the browser refuses the preflight and
  // the Mini App never gets to send its session token at all.
...
// ---------------------------------------------------------------------------
// Telegram Mini App — B-6
// ---------------------------------------------------------------------------

test('the two implemented TMA routes match, and only as POST', () => {
...
test('every TMA route without a handler is refused by the proxy', async () => {
  // The other twelve of the fourteen. Allowlisting them early would send a Mini
  // App request into an admin catch-all upstream and get 401 ADMIN_AUTH_REQUIRED
  // back — a diagnosis that sends the reader looking in the wrong service.
  const notYet = [
    ['GET', '/api/v1/tma/me'],
    ['GET', '/api/v1/tma/me/orders'],
    ['GET', '/api/v1/tma/me/orders/active'],
    ['POST', '/api/v1/tma/orders'],
    ['GET', '/api/v1/tma/orders/tok_abc/status'],
    ['GET', '/api/v1/tma/esims'],
    ['GET', '/api/v1/tma/esims/11111111-2222-3333-4444-555555555555'],
    ['POST', '/api/v1/tma/esims/11111111-2222-3333-4444-555555555555/activation'],
    ['POST', '/api/v1/tma/esims/11111111-2222-3333-4444-555555555555/usage/refresh'],
    ['POST', '/api/v1/tma/identity/email/request'],
    ['POST', '/api/v1/tma/identity/email/confirm'],
    ['POST', '/api/v1/tma/identity/email/revoke'],
  ];

  for (const [method, path] of notYet) {
    assert.equal(matchRoute(method, path), null, `${method} ${path} is reachable too early`);

    reset();
    const res = await fn.handler({ httpMethod: method, path, headers: {}, queryStringParameters: {} });
    assert.equal(res.statusCode, 404);
    assert.equal(captured.length, 0, `${path} was forwarded upstream`);
  }
});

test('a made-up TMA path is refused, and the prefix is not a wildcard', async () => {
  for (const path of [
    '/api/v1/tma',
    '/api/v1/tma/',
    '/api/v1/tmaxyz/session',
    '/api/v1/tma/session/../../admin/sync',
    '/api/v1/tma/session/extra',
    '/api/v1/tma/sessions',
  ]) {
    assert.equal(matchRoute('POST', path), null, `${path} is reachable`);
  }
});

test('Authorization reaches the upstream socket', async () => {
  // Asserting the constant contains the name would prove the constant. This
  // asserts the header is on the request that goes out.
  reset();
  await call({
    httpMethod: 'POST',
  ...
  assert.equal(captured.length, 1);
  assert.equal(captured[0].headers.authorization, 'Bearer ' + 'a'.repeat(64));
});

test('a lowercase authorization header is forwarded too', async () => {
  reset();
  await call({
    httpMethod: 'POST',
    path: '/api/v1/tma/session/revoke',
  ...
  assert.equal(captured[0].headers.authorization, 'bearer abc123');
});

test('Authorization travels on the legacy routes too, and that changes nothing', async () => {
  // The header is allowlisted globally rather than per route. Worth asserting
  // rather than assuming: the retail routes ignore it, and the dealer-aware
  // catalogue that would react to it is not in the allowlist at all.
  reset();
  await call({
    httpMethod: 'GET',
    path: '/api/v1/retail/packages',
  ...
  assert.equal(captured[0].headers.authorization, 'Bearer dealer-token');
  assert.equal(matchRoute('GET', '/api/v1/packages'), null, 'the dealer-aware catalogue must stay unreachable');
});

test('the Authorization VALUE never appears in a log line', async () => {
  reset();
  const lines = [];
  const realLog = console.log;
  console.log = (...args) => lines.push(args.join(' '));
  ...
  try {
    await call({
      httpMethod: 'POST',
      path: '/api/v1/tma/session/revoke',
      headers: { Authorization: 'Bearer LIVE-SESSION-TOKEN-VALUE' },
      body: '{}',
    });
    // And on a refusal, where a different code path builds the log line.
    await call({
      httpMethod: 'POST',
      path: '/api/v1/tma/me',
      headers: { Authorization: 'Bearer LIVE-SESSION-TOKEN-VALUE' },
  ...
  assert.ok(!lines.join('\n').includes('LIVE-SESSION-TOKEN-VALUE'));
  assert.ok(!lines.join('\n').toLowerCase().includes('authorization'));
});

test('masking covers the TMA paths that will carry a token', () => {
  // Added ahead of stage D. Forgetting masking later is easier than adding it now.
  for (const path of [
    '/api/v1/tma/orders/SECRET-TOKEN/status',
    '/api/v1/tma/esims/SECRET-TOKEN/activation',
  ]) {
    const logged = logPath(path, null);
    assert.ok(!logged.includes('SECRET-TOKEN'), `${path} logged its token`);
    assert.ok(logged.includes('{token}'));
...
test('the preflight a Mini App browser actually sends is accepted', async () => {
  reset();
  const res = await call({
    httpMethod: 'OPTIONS',
